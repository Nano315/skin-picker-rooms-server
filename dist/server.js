...
app.use((0, cors_1.default)()); // Note: In production, configure origin explicitly
...
function handleMemberLeave(room, memberId, reason) {
...
function recomputeSynergy(room) {
...
app.post("/rooms/:code/bots", (req, res) => {
    ...
    const namePrefix = typeof body.namePrefix === "string" && body.namePrefix.trim()
        ...
        : "Bot";
    const forcedChampionId = typeof body.championId === "number" ? body.championId : undefined;
    const forcedSkinId = typeof body.skinId === "number" ? body.skinId : undefined;
    const forcedChromaId = typeof body.chromaId === "number" ? body.chromaId : undefined;
    const createdBots = [];
    console.log(`[dev-bots] adding ${count} bot(s) to room ${room.id} (code=${room.code})`);
    ...
    for (let i = 0; i < count; i++) {
        ...
        const bot = {
            ...
            championId: forcedChampionId !== undefined ? forcedChampionId : (0, crypto_1.randomInt)(1, 201),
            championAlias: "",
            skinId: forcedSkinId !== undefined ? forcedSkinId : (0, crypto_1.randomInt)(1000, 999999),
...
io.on("connection", (socket) => {
    ...
    socket.on("join-room", ({ roomId, memberId }) => {
        ...
        if (!member)
            return;
        console.log(`[socket] ${socket.id} join room ${roomId} as member ${memberId}`);
        socket.join(roomId);
    ...
    socket.on("owned-options", (payload) => {
        ...
        console.log(`[owned-options] member=${memberId} room=${roomId} options=${member.options?.length}`);
    ...
    socket.on("leave-room", ({ roomId, memberId }) => {
        ...
        if (!room.members.has(memberId))
            return;
        console.log(`[socket] ${socket.id} explicit leave room ${roomId} (member ${memberId})`);
    ...
    socket.on("disconnect", () => {
        ...
        if (!room)
            return;
        console.log(`[socket] ${socket.id} disconnected from room ${roomId} (member ${memberId})`);
    ...
    socket.on("request-group-reroll", (payload) => {
        ...
        if (room.ownerId !== memberId) {
            console.warn(`[group-reroll] non-owner tried to reroll: member=${memberId} room=${roomId}`);
            return;
        }
        const synergy = room.synergy;
        if (!synergy)
            return;
        const entry = synergy.colors.find((c) => c.type === type && c.color === color && c.combinationCount > 0);
        if (!entry) {
            console.warn(`[group-reroll] no synergy entry for color=${color} in room=${roomId}`);
            return;
        }
        const picks = [];
        ...
        for (const m of room.members.values()) {
            if (!m.isReady)
                continue; // On ne change pas le skin de ceux qui ne sont pas prêts
            const opts = (m.options ?? []).filter((o) => o.auraColor === color);
            if (!opts.length) {
        ...
        console.log(`[group-reroll] applying combo color=${color} in room=${roomId}, picks=${picks.length}`);
